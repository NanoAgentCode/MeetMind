    ...
    <Spin spinning={loading}>
      ...
      <section className="management-section">
        ...
        {providers.length ? <div className="provider-grid">{providers.map((item) => <article className="provider-card" key={item.id}>
          ...
          <div className="card-actions"><Button icon={<UnorderedListOutlined />} loading={loadingModels === item.id} onClick={() => fetchModels(item.id, true)}>获取模型</Button><Button icon={<ApiOutlined />} loading={testingProvider === item.id} onClick={() => testProvider(item)}>测试连接</Button><Button icon={<EditOutlined />} onClick={() => openProvider(item)}>编辑</Button><Button danger type="text" icon={<DeleteOutlined />} onClick={() => removeProvider(item)}>删除</Button></div>
