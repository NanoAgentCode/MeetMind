...
import { ApiOutlined, CloudServerOutlined, DeleteOutlined, EditOutlined, PlusOutlined, RobotOutlined } from '@ant-design/icons'
...
import {
  ...
  listModelConfigs, listModelProviders, testModelProvider, updateModelConfig, updateModelProvider,
} from './api'
...
export default function ModelManagement() {
  ...
  function openModel(item?: ModelConfig) {
    ...
    } : { ...emptyModel, provider_id: providers[0]?.id || '' })
  }
  ...
  return <>
    ...
    <Spin spinning={loading}>
      ...
      <section className="management-section">
        ...
        {providers.length ? <div className="provider-grid">{providers.map((item) => <article className="provider-card" key={item.id}>
          ...
          <div className="card-actions"><Button icon={<ApiOutlined />} loading={testingProvider === item.id} onClick={() => testProvider(item)}>测试连接</Button><Button icon={<EditOutlined />} onClick={() => openProvider(item)}>编辑</Button><Button danger type="text" icon={<DeleteOutlined />} onClick={() => removeProvider(item)}>删除</Button></div>
        </article>)}</div> : <Empty description="还没有供应商配置"><Button type="primary" onClick={() => openProvider()}>添加第一个供应商</Button></Empty>}
      </section>
      <section className="management-section">
        <div className="management-head"><div><h2>模型配置</h2><p>每种类型最多设置一个默认模型，业务调用将优先使用默认配置</p></div><Button icon={<PlusOutlined />} disabled={!providers.length} onClick={() => openModel()}>添加模型</Button></div>
        {models.length ? <div className="model-list">{models.map((item) => <article key={item.id}>
          ...
          <div className="card-actions"><Button type="text" icon={<EditOutlined />} onClick={() => openModel(item)} /><Button danger type="text" icon={<DeleteOutlined />} onClick={() => removeModel(item)} /></div>
    ...
    <Modal title={modelEditing ? '编辑模型配置' : '添加模型配置'} open={!!modelDraft} onCancel={() => setModelDraft(null)} onOk={saveModel} confirmLoading={saving} okText="保存" cancelText="取消">
      {modelDraft && <div className="config-form">
        <label>所属供应商</label><Select value={modelDraft.provider_id} options={providers.map((item) => ({ value: item.id, label: item.name }))} onChange={(provider_id) => setModelDraft({ ...modelDraft, provider_id })} />
        ...
        <label>模型标识</label><Input value={modelDraft.model_id} placeholder="例如：gpt-4o-mini" onChange={(e) => setModelDraft({ ...modelDraft, model_id: e.target.value })} />
